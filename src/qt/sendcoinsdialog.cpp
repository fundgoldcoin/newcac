    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a ClassicAsset address (e.g. CH1PFFgK3KtzaV9ce467nq5m5Vis1E4hZ)"));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
		...
		{ 
#if QT_VERSION < 0x050000 
        formatted.append(tr("<b>%1</b> in %4 blocks of %5 CAC each to %2 (%3)?").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BTC, rcp.amount),  
			Qt::escape(rcp.label),  
			rcp.address,  
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid ClassicAsset address"));
        }
        else
        {
